...
cumLexc_ad2bis <- function(z_tau, z_alpha, x, theta)
{
  n_z_tau <- ncol(z_tau)
  ...
  cumhaz[which(x <= tau)] <- (tau)[which(x <= tau)] * beta(alpha[which(x <= tau)], beta2) * stats::pbeta(u[which(x <= tau)], alpha[which(x <= tau)], beta2)
  cumhaz[which(x > tau)] <- (tau)[which(x > tau)] * beta(alpha[which(x > tau)], beta2) * stats::pbeta(1, alpha[which(x <= tau)], beta2)
