...
lexc_m1_ad <- function(z_tau, z_alpha, z_c, x, theta) {
  ...
  if (n_z_tau > 0 & n_z_alpha > 0) {
    ...
    if (n_z_c > 0) {
      ...
      c_par <- exp(-(rowSums(c_par0 + z_c %*% c_k))^10)
    }else{
      c_par <- c_par0 <- exp((-theta[(n_z_alpha + 2 + n_z_tau + 1 + 1)])^10)
    }
    alpha <- (rowSums((alpha0 + z_alpha %*% alpha_k)))

    tau <- (rowSums(tau0 + z_tau %*% tau_z))
    u <- x / (tau)
    haz <- ifelse((x >= 0 & x <= (tau)),
                  ((u ^ ((alpha) - 1) * (1 - u) ^ ((beta - 1))) + (c_par)),
                  (c_par))
