lexc_ad3bis <- function(z_tau, z_alpha, x, theta) {
  n_z_tau <- ncol(z_tau)
  ...
  u <- x / (tau)
  haz <- rep(0, length(x))
  haz[which(x <= tau)] <- (1 - p) * (((u[which(x <= tau)]) ^ ((alpha[which(x <= tau)]) - 1) * (1 - u[which(x <= tau)]) ^ (beta - 1))) + ( exp(gamma)*exp(lambda)*((x[which(x <= tau)])^(exp(gamma) - 1))) * p
  haz[which(x > tau)] <- 0 +  (exp(gamma)*exp(lambda)*((x[which(x > tau)])^(exp(gamma) - 1)))
  return(haz)
}
